...
published 0.1.6 tarball is 47 files, all first-party.
...
| Capability | Yoke (npm 0.1.6) | @playwright/mcp 0.0.79 (2026-08) | chrome-devtools-mcp 1.7.0 (pub. 2026-08-10) | BrowserMCP: server 0.1.3 (2025-04-11), extension 1.3.4 (2025-05-07), unmaintained |
| --- | --- | --- | --- | --- |
...
3. Since 0.1.4 each profile has its own endpoint, `list_browsers` names them, and the tab tools route by tab id, described under Several Chrome profiles below. On 0.1.2 a second profile with the extension loaded could take the single per-user endpoint silently, and 0.1.3 made one profile win deterministically. Two different browsers, Chrome and Brave say, number tabs independently, so a tab id both report is refused rather than guessed.
