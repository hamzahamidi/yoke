...
- 0.1.4: one endpoint per Chrome profile, named after an id the extension mints and keeps. `list_browsers` names each connected profile, `list_tabs` covers all of them, tab tools route by tab id, and `open_tab` takes `browser` when there is a choice. Closes the wrong-profile case in #6 and #8, where a single per-user endpoint was won by whichever profile connected first.
- 0.1.6: an endpoint is held only by a host that can serve it. Chrome can leave a host holding a pipe whose service worker is gone, and 0.1.5 let that host claim a socket which then accepted every caller and answered each one with a timeout, counted as a browser with no tabs, and kept the endpoint from the profile that could have served it (#19). A host now claims nothing unless the extension answers, gives the endpoint up when the extension stops answering, and waits for an incumbent to let go rather than unlinking a name a live server still holds.
- 0.1.5: a profile that loses the host comes back. Chrome stops an idle service worker, which takes the scheduled retry with it, so 0.1.4 could sit disconnected until somebody clicked the icon (#12). Browsing now wakes it, the backoff doubles as it always claimed to rather than staying at two seconds, and the message says to click the icon before it says to reinstall. Recovery on a browser nobody is touching is still open, in #15.
